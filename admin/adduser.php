    ...
    <form role="form" action="" method="post" enctype="multipart/form-data">
     <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
      <tr>
       ...
       <td>
        <input type="text" name="yhm" id="yhm" value="" size="80" class="inpMain" required="required"/>
       </td>
      </tr>
       <tr>
       <td width="90" align="right">密码</td>
       <td>
        <input type="text" id="mm" name="mm" value="" size="80" class="inpMain" required="required"/>
       </td>
      </tr>
      <tr>
       <td align="right">Email地址</td>
       <td>
        <input type="email" id="email" name="email" value="" size="50" class="inpMain" required="required"/>
       </td>
      </tr>
      <tr>
		...
		else
		{
		 	...
		 	if($_FILES['myfile']['type']=="image/jpeg"||$_FILES['myfile']['type']=="image/png"||$_FILES['myfile']['type']=="image/pjpeg")//判断文件格式是否为JPEG
			{
		 		...
		 		else
		 		{
					...
					$sql="select *from user where images='".$filename."'";
					$result=mysql_query($sql);
					if(mysql_num_rows($result)>0){
