...
if(isset($_POST['xgtp']))
{
	...
	if($_FILES['myfile']['type']=="image/jpeg"||$_FILES['myfile']['type']=="image/png"||$_FILES['myfile']['type']=="image/pjpeg")//判断文件格式是否为JPEG
	{
		...
		else
		{
			...
			$sql="select *from user where image='".$filename."'";
			$result=mysql_query($sql);
			...
			{	
				...
				if(is_uploaded_file($tmp_filename))			//判断是否通过HTTP POST上传
				{
					...
					{
						 	$sql="update user set image='".$filename."' where id='".$user."'";
						 	$result=mysql_query($sql);
						 	if($result)
						 	{
...
if(isset($_POST['submit']))
{
	...
	if($row['username'] != $_POST['yhm'])
	{
        ...
        else
        {
            ...
            $sql="update user set username='".$username."',userpassword='".$userpassword."',email='".$email."',status='".$status."',sex='".$sex."',place='".$place."',integral='".$integral."',address='".$address."' where id='".$user."'";
            $result=mysql_query($sql);
            echo $sql;
            if($result)
            {
