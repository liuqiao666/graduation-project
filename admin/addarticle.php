			...
			else
			{
				...
				else
				{
				    ...
				    if($_FILES["file"]["type"]=="audio/mp3" || $_FILES["file"]["type"]=="mp3/mp3")
				    {
						...
						$sql="select *from adarticle where music='".$file."'";
						$result=mysql_query($sql);
						if(mysql_num_rows($result)>0){
						 		...
						 		else
						 		{
									...
									{	
										...
										if(is_uploaded_file($tmp_filename))			//判断是否通过HTTP POST上传
										{
											...
											{
												 	$sql="insert into adarticle (articleid,name,music,mdescribes,classes,articleimg,articledescribes,time) values('".$articleid."','".$name."','".$file."','".$mdescribes."','".$classes."','".$filename."','".$articledescribes."','".$day."')";
												 	$result=mysql_query($sql);
												 	if($result)
												 	{
												     	...
												     	echo "<script>alert('音乐分享文章发布失败!');</script>;";
													echo $sql;	
											}
											else
