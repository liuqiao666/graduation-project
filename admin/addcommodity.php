  ...
  <h3><a href="commodity.php" class="actionBtn">商品列表</a>添加商品</h3>
    <form action="" method="post" enctype="multipart/form-data">
     <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">     
       ...
       <td>
        <input type="text" name="number" value="" size="50" class="inpMain" required="required"/>
       </td>
      </tr>
      <tr>
       ...
       <td>
        ...
        <!-- /KindEditor -->
        <textarea id="content" name="description" style="width:780px;height:400px;" class="textArea" required="required"></textarea>
       </td>
      </tr>
       <tr>
       <tr>
       ...
       <td>
        <input type="text" name="price" value="" size="50" class="inpMain" required="required"/>
       </td>
      </tr>
       <tr>
       <td align="center">库存 </td>
       <td>
        <input type="text" name="stock" value="<?php echo $row['stock'];?>" size="50" class="inpMain" required="required"/>
       </td>
      </tr>
      <tr>
       <td></td>
		...
		else
		{
		 	...
		 	if($_FILES['myfile']['type']=="image/jpeg"||$_FILES['myfile']['type']=="image/png"||$_FILES['myfile']['type']=="image/pjpeg")//判断文件格式是否为JPEG
			{
		 		...
		 		else
		 		{
					...
					$sql="select *from commodity where img='".$filename."'";
					$result=mysql_query($sql);
					if(mysql_num_rows($result)>0){
