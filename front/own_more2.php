...
<?php
 ...
 if (!empty($id)) 
 { 
	$sql="select * from collect where username='".$id."' and kind=1";
 } 
 else
 {
       	 	...
       	 	<div class="chart_artsit">
                	<ul>
						...
						else 
						{
						 ...
						 if (!empty($id)) 
 					   	 { 
							$sql="select * from collect where username='".$id."' and kind=1 limit $offset,$page_size";
						 } 
						 else
						 {
							$sql="select * from collect where username='".$username."' and kind=1 limit $offset,$page_size";
						 }
						 $result=mysql_query($sql);
                        	...
                        	<div class="chart_artsit_item">
                             ...
                             <a class="buddy" href="periodical_open.php?particleid=<?php echo $row->articleid?>" title=""><img src="../images/adminwenzhang/<?php echo $row->img?>" width="55" height="55"/></a>
                              <div class="private_act">
                    			<a class="removeIT" href="delete.php?number=<?php echo $row->articleid?>&username=<?php echo $row->username?>"  onclick="return confirm('确认删除？');"></a><!--删除-->
                              </div><!--删除完-->
                            </div>
...
$(function(){
	...
	var song = [
		<?PHP 
						 ...
						 $sql="select img,music,collect.name,collect.articleid from adarticle,collect where collect.articleid = adarticle.articleid and collect.username='".$username."' and kind = '1'";
						 $result=mysql_query($sql);
						 while($row=mysql_fetch_object($result))
						 {
						 ?>
		{
			...
			'cover' : '../images/adminwenzhang/<?php echo $row->img;?>',
