...
$sql="select * from collect where kind=0 and username='".$username."' and articleid='".$number."'";
...
else
{
	...
	$img=$row['articleimg']; 
	$collect=$row['collect']+1; 
 	$sql="insert into collect (username,tjrname,articleid,name,img,kind) values('".$username."','".$tjrname."','".$number."','".$name."','".$img."','0')";
	$result=mysql_query($sql);
 	if($result)
 	{
		$sql="update usarticle set collect='".$collect."' where articleid='".$number."'";
